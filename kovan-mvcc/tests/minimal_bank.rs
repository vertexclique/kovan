...
fn test_simple_concurrent_transfer() {
    ...
    // Initialize accounts
    {
        ...
        for i in 0..num_accounts {
            txn.write(&format!("acc_{}", i), 1000u64.to_le_bytes().to_vec()).unwrap();
        }
        txn.commit().unwrap();
    }
        ...
        handles.push(thread::spawn(move || {
            ...
            for _ in 0..num_transfers { // Changed loop variable to _
                ...
                loop {
                    ...
                    // Write phase
                    if txn.write(&k_from, (b_from - 10).to_le_bytes().to_vec()).is_err() {
                        continue;
                    }
                    if txn.write(&k_to, (b_to + 10).to_le_bytes().to_vec()).is_err() {
                        continue;
                    }
    ...
    assert_eq!(total, 1000 * num_accounts as u64, "Money was created or destroyed!");
}
