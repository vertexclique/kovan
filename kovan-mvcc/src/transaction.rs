...
pub struct InMemoryTxnManager {
    ...
    records: HopscotchMap<u128, Arc<TransactionRecord>>,
}

impl InMemoryTxnManager {
    pub fn new() -> Self {
        Self {
            records: HopscotchMap::new(),
        }
    }
    ...
    pub fn set_state(&self, txn_id: u128, state: TxnState, ts: Option<u64>) {
        if let Some(rec) = self.records.get(&txn_id) {
            if let Some(t) = ts {
                rec.commit_timestamp.store(t, Ordering::Release);
            }
            rec.state.store(state as u8, Ordering::Release);
            std::sync::atomic::fence(Ordering::SeqCst); // Enforce visibility
        } else {
    ...
    pub fn check_status(&self, txn_id: u128) -> (TxnState, Option<u64>) {
        ...
        match self.records.get(&txn_id) {
            Some(rec) => {
                ...
                let ts_val = rec.commit_timestamp.load(Ordering::Acquire);
