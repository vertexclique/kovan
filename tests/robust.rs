...
use kovan::{pin, retire, Atomic, BirthEra, RetiredNode, current_era};
...
fn test_birth_era_ordering() {
    ...
    assert!(era1.is_older_than(era2.value()), "Earlier era should be older");
    assert!(!era2.is_older_than(era1.value()), "Later era should not be older");
}
...
fn test_stalled_thread_handling() {
    ...
    println!("Stalled thread handling: {} operations completed", total_ops);
